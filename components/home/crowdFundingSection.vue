      ...
      <div class="flex items-center justify-between mb-12">
        <div class="flex-1 text-center">
          <span class="text-4xl md:text-5xl font-bold relative inline-block after:content-[''] after:absolute after:left-1/2 after:-translate-x-1/2 after:-bottom-2 after:w-4/5 after:h-0.5 after:bg-sec">
            <TitleLg title="Crowd Funding" class="!text-4xl !md:text-5xl" />
          </span>
        </div>
        <NuxtLink 
          to="/crowd-funding" 
...
const transformAd = (ad) => ({
  id: ad._id || ad.id,
  image: ad.mainImage
    ? `${API_BASE_URL.replace('/v1', '')}/uploads/ad_images/${ad.mainImage}`
  ...
  price: ad.price || 0,
  currency: 'LKR',
  section: ad.section || 'crowdfunding',
  yearEstablished: ad.yearEstablished,
  platformType: ad.platformType,
  platformFollowers: ad.platformFollowers,
  avgRevenueMonthly: ad.avgRevenueMonthly,
  avgProfitYearly: ad.avgProfitYearly,
  equityPercentage: ad.equityPercentage,
...
const fetchTrending = async () => {
  try {
    ...
    const response = await apiService.request(API_ENDPOINTS.ADS.GET_BY_SECTION('crowdfunding', 10))
    ...
    // Use database results only — no local fallback
