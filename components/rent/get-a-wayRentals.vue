      ...
      <div class="flex items-center justify-between mb-12">
        <div class="flex-1 text-center">
          <span class="text-4xl md:text-5xl font-bold relative inline-block after:content-[''] after:absolute after:left-1/2 after:-translate-x-1/2 after:-bottom-2 after:w-4/5 after:h-0.5 after:bg-sec">
            <TitleLg title="Get-A-Way Rentals" class="!text-4xl !md:text-5xl" />
          </span>
        </div>
        <NuxtLink 
          to="/rent" 
...
const fetchTrending = async () => {
  try {
    ...
    // attempt to find a category for "get away" rentals
    let mapped = []
    try {
      const catResp = await apiService.request(API_ENDPOINTS.CATEGORIES.GET)
      const getawayCat = (catResp?.categories || []).find(c => {
        const name = (c.name || '').toLowerCase()
        return name.includes('get-a-way') || name.includes('getaway') || name.includes('get a way')
      })
      if (getawayCat) {
        const getawayCatId = getawayCat?.id || getawayCat?._id
        if (getawayCatId) {
          const adResp = await apiService.request(API_ENDPOINTS.ADS.GET_BY_CATEGORY(getawayCatId))
          const results = adResp?.results || adResp || []
          mapped = results.map(transformAd)
        }
      }
    } catch (e) {
      console.warn('getaway category lookup failed', e)
    }
