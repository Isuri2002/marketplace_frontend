      ...
      <div class="flex items-center justify-between mb-12">
        <div class="flex-1 text-center">
          <span class="text-4xl md:text-5xl font-bold relative inline-block after:content-[''] after:absolute after:left-1/2 after:-translate-x-1/2 after:-bottom-2 after:w-4/5 after:h-0.5 after:bg-sec">
            <TitleLg title="Car Rentals" class="!text-4xl !md:text-5xl" />
          </span>
        </div>
        <NuxtLink 
          to="/rent" 
...
const fetchTrending = async () => {
  try {
    ...
    // first try to load car category and then fetch ads for that category
    let mapped = []
    try {
      const catResp = await apiService.request(API_ENDPOINTS.CATEGORIES.GET)
      const carCat = (catResp?.categories || []).find(c =>
        c.name?.toLowerCase().includes('car') ||
        c.name?.toLowerCase().includes('vehicle')
      )
      if (carCat) {
        const carCatId = carCat?.id || carCat?._id
        if (carCatId) {
          const adResp = await apiService.request(API_ENDPOINTS.ADS.GET_BY_CATEGORY(carCatId))
          const results = adResp?.results || adResp || []
          mapped = results.map(transformAd)
        }
      }
    } catch (e) {
      // silently ignore category lookup failure and fall back to section
      console.warn('car category lookup failed', e)
    }

    // if no car‑specific ads found, fall back to generic rent section
